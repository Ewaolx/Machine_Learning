...
for i in range(0, 10):
    total = len(glob.glob1("proj3_images\\Numerals\\" + str(i), "*.png"))
    j = 0
    for filename in os.listdir("proj3_images\\Numerals\\" + str(i)):
        if filename.endswith('.png'):
            pic = Image.open("proj3_images\\Numerals\\" + str(i) + "\\" + filename).convert('L')
...
print('Logistic Regression test accuracy:' + str(sess.run(accuracy, feed_dict={x: X_images, y_: X_labels})))
...
with tf.Session() as sess:
   ...
   print('mlp with one hidden layer test accuracy:' + str(sess.run(accuracy, feed_dict={x: X_images, y_: X_labels})))
